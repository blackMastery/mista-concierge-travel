...
**What you can do:** full CRUD on tours (incl. gallery images uploaded to Storage, highlights, itinerary, inclusions, activity tags), destinations, testimonials, and team; moderate reviews (publish/unpublish/delete); edit the `site_content` brand blocks (banner, hero stats, pillars, values, certs, footer links); and work the leads inbox — booking requests (status), contact messages (read/archive/delete), and newsletter subscribers (CSV export). Edits revalidate the affected public pages so they show immediately.

**Security:** writes go through the signed-in admin's session — no service-role key. `requireAdmin()` ([lib/admin.ts](lib/admin.ts)) gates every admin page and action, and RLS (`is_admin()`) enforces it at the database, so non-admins are blocked even outside the UI. Non-admins hitting `/admin` are redirected (`/` if signed in, `/login` if not).
